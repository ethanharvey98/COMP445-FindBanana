...
public class BFSStage implements Comparable {
	// Variable dictionary
	...
	// Constructor
	public BFSStage(int number_of_rooms, 
			int monkey_room, 
			int chair_room, 
			int g, 
			boolean on_chair,
	...
	public BFSStage[] nextStages() {
		...
		BFSStage[] next = new BFSStage[length];
		
		// Add stages
		int incrementor = 0;
		if (can_move_left) {
			next[incrementor] = new BFSStage(number_of_rooms, monkey_room-1, chair_room, this.g+1, false, false);
			incrementor += 1;
		}
		if (can_move_right) {
			next[incrementor] = new BFSStage(number_of_rooms, monkey_room+1, chair_room, this.g+1, false, false);
		...
		if (can_push_left) {
			next[incrementor] = new BFSStage(number_of_rooms, monkey_room-1, chair_room-1, this.g+5, false, false);
			incrementor += 1;
		}
		if (can_push_right) {
			next[incrementor] = new BFSStage(number_of_rooms, monkey_room+1, chair_room+1, this.g+5, false, false);
		...
		if (can_climb) {
			next[incrementor] = new BFSStage(number_of_rooms, monkey_room, chair_room, this.g+2, true, false);
			incrementor += 1;
		}
		if (can_grab) {
			next[incrementor] = new BFSStage(number_of_rooms, monkey_room, chair_room, this.g+1, true, true);
			incrementor += 1;
		}
	...
	public int compareTo(Object arg0) {
		return this.getF() - ((BFSStage)arg0).getF();
	}
